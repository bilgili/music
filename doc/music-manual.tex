...
these views.  MUSIC uses \emph{shared indices}\index{shared index} to
enumerate the entire set of data to be sent over the connection while
\emph{application indices}\index{application index} enumerate the
...
application should be mapped to the shared indices common for
sender and receiver.  The applications indices can be local within
each MPI process or global over the application as indicated by the
\lstinline|type| parameters (\lstinline|MUSIC::Index::LOCAL| or
\lstinline|MUSIC::Index::GLOBAL|).
...
\begin{parameters}
  ...
  \lstinline|id| & the application index of the event \\
\end{parameters}
...
Since the simulation time is updated when \lstinline|tick| is called,
this means that conceptually, time in not changing between these
calls.  When sending continuous values this implies that the
application must ensure that data arrays mapped for output are filled
with values relevant for the time before the tick increment.  After
the tick increment, data arrays mapped for input will be filled with
data relevant for the time after the tick increment.
