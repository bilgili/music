...
\begin{head}
    ...
    void cont_input_port::map (cont_data* map, int max_latency)

    void event_output_port::map (event_data* map)

    void event_input_port::map (event_data* map, int max_latency)
\end{head}
\begin{tabular}{ll}
  map         & the data map associated with the port \\
  max\_latency & maximum latency for incoming data (glocal clock ticks)
  \\
\end{tabular}
